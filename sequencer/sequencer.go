...
func (s *Sequencer) checkAndRegisterProcesses() {
	...
	for _, processID := range procesList {
		...
		if err != nil {
			log.Warnw("failed to get process for registration", "processID", processID, "error", err)
			continue
		}
...
func (s *Sequencer) AddProcessID(processID types.ProcessID) {
	if s.pids.Add(processID) {
		log.Infow("process ID registered for sequencing", "processID", processID)
	}
}
...
func (s *Sequencer) DelProcessID(processID types.ProcessID) {
	if s.pids.Remove(processID) {
		log.Infow("process ID unregistered from sequencing", "processID", processID)
	}
}
