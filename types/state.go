...
// MapHashToVoteID returns a VoteID on the upper half of the StateKey space
// VoteID = VoteIDMin + (hash mod VoteIDMin)
func MapHashToVoteID(hash *big.Int) VoteID {
	return VoteID(params.VoteIDMin +
		new(big.Int).Mod(hash, new(big.Int).SetUint64(params.VoteIDMin)).Uint64())
}

// CalculateBallotIndex returns a BallotIndex on the lower half of the StateKey space,
// below VoteIDMin and above the ConfigMax space.
//
...
func CalculateBallotIndex(address *big.Int, censusIndex uint64) BallotIndex {
	if censusIndex > params.CensusIndexMax {
		panic("censusIndex too big") // TODO: return error instead
	}
	censusIndexShifted := censusIndex * (1 << params.CensusAddressBitLen)
	addressTruncated := TruncateToLowerBits(address, params.CensusAddressBitLen)
	return BallotIndex(params.BallotMin + censusIndexShifted + addressTruncated.Uint64())
}
