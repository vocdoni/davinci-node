...
func (bs *BlobSidecar) String() string {
	return fmt.Sprintf("BlobSidecar{Index: %d, Commitment: %x}", bs.Index, bs.Commitment)
}

const (
	BlobSidecarVersion0 = gethtypes.BlobSidecarVersion0
	BlobSidecarVersion1 = gethtypes.BlobSidecarVersion1
)
...
func ComputeBlobTxSidecar(version byte, raw [][]byte) (*BlobTxSidecar, error) {
	n := len(raw)
	if n == 0 {
