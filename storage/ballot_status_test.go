...
func TestBallotStatus(t *testing.T) {
	...
	status, err := st.GetBallotStatus(pidBytes, voteID1)
	c.Assert(err, qt.IsNil)
	c.Assert(status, qt.Equals, BallotStatusPending)

	// Test status name
	c.Assert(GetBallotStatusName(status), qt.Equals, "pending")
	...
	_, err = st.GetBallotStatus(pidBytes, []byte("nonexistent"))
	...
	status1, err := st.GetBallotStatus(pidBytes, voteID1)
	c.Assert(err, qt.IsNil)
	c.Assert(status1, qt.Equals, BallotStatusSettled)

	status2, err := st.GetBallotStatus(pidBytes, voteID2)
	c.Assert(err, qt.IsNil)
	c.Assert(status2, qt.Equals, BallotStatusSettled)

	// voteID3 should still be in "processed" state
	status3, err := st.GetBallotStatus(pidBytes, voteID3)
	c.Assert(err, qt.IsNil)
	...
	status1Value, err = st.GetBallotStatus(pidBytes, voteID1)
	c.Assert(err, qt.IsNil)
	...
	status2Value, err = st.GetBallotStatus(pidBytes, voteID2)
	c.Assert(err, qt.IsNil)
