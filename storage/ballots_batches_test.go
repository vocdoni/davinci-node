...
func TestMarkStateTransitionBatchFailedDoesNotRequeueMutatedPendingBatch(t *testing.T) {
	...
	err = stg.MarkStateTransitionBatchFailed(stateTransitionKey)
	c.Assert(err, qt.IsNil)
...
func TestMarkStateTransitionBatchFailedMarksBallotsErrorWithoutPendingBatch(t *testing.T) {
	c := qt.New(t)
	stg := newTestStorage(t)
	defer stg.Close()
	...
	stb := &StateTransitionBatch{
		ProcessID: processID,
		Ballots: []*AggregatorBallot{
			{VoteID: voteID, Address: big.NewInt(7001)},
		},
		Inputs: StateTransitionBatchProofInputs{
			RootHashBefore: big.NewInt(30),
			RootHashAfter:  big.NewInt(31),
			CensusRoot:     big.NewInt(32),
		},
	}
	c.Assert(stg.PushStateTransitionBatch(stb), qt.IsNil)
...
func TestMarkStateTransitionBatchFailedUsesDecodedProcessIDForCleanup(t *testing.T) {
	c := qt.New(t)
	stg := newTestStorage(t)
	defer stg.Close()
	...
	batch := &AggregatorBallotBatch{
		ProcessID: processID,
		Ballots: []*AggregatorBallot{
			{VoteID: voteID, Address: big.NewInt(8001)},
		},
	}
	c.Assert(stg.PushAggregatorBatch(batch), qt.IsNil)
	...
	stb := &StateTransitionBatch{
		...
		Ballots:   retrievedBatch.Ballots,
		Inputs: StateTransitionBatchProofInputs{
			RootHashBefore: big.NewInt(40),
			RootHashAfter:  big.NewInt(41),
			CensusRoot:     big.NewInt(42),
		},
	}
	c.Assert(stg.PushStateTransitionBatch(stb), qt.IsNil)
