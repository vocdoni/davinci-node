...
const (
	BallotStatusPending    = 0
	BallotStatusVerified   = 1
	BallotStatusAggregated = 2
	BallotStatusProcessed  = 3
	BallotStatusSettled    = 4
	BallotStatusError      = 5
)
...
// GetBallotStatus returns the status of a ballot for a given processID and voteID.
// Returns ErrNotFound if the ballot status doesn't exist.
func (s *Storage) GetBallotStatus(processID, voteID []byte) (int, error) {
	s.globalLock.Lock()
...
func GetBallotStatusName(status int) string {
