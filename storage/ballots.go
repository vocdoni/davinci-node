...
func (s *Storage) CountPendingBallots() int {
	...
	if err := rd.Iterate(nil, func(k, _ []byte) bool {
		...
		if s.isReserved(ballotPrefix, k) {
			return true
		}
		count++
		return true
...
func (s *Storage) PullVerifiedBallots(processID types.ProcessID, numFields int) ([]*VerifiedBallot, [][]byte, error) {
	...
	if err := rd.Iterate(pidBytes, func(k, v []byte) bool {
		...
		// Skip if already reserved
		if s.isReserved(verifiedBallotPrefix, k) {
			return true
		}
	...
	for i, k := range keys {
		if err := s.setReservation(verifiedBallotPrefix, k); err != nil {
...
func (s *Storage) nextPendingBallot() (*Ballot, types.VoteID, error) {
	...
	// set reservation
	if err := s.setReservation(ballotPrefix, b.VoteID.Bytes()); err != nil {
		return nil, 0, ErrNoMoreElements
	}
