...
func (a *API) voteByAddress(w http.ResponseWriter, r *http.Request) {
	...
	// Generate the census proof of the address to get the voter index
...
func (a *API) newVote(w http.ResponseWriter, r *http.Request) {
	...
	// voter weight. For Merkle censuses, always derive VoterIndex from the
	...
	isOverwrite, err := state.IndexContainsBallot(a.storage.StateDB(), *process.ID,
		types.CalculateBallotIndex(vote.CensusProof.VoterIndex))
	if err != nil {
