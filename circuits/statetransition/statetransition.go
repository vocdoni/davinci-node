...
// real and 0 otherwise. It uses a latch logic to avoid expensive comparisons
// inside the loops.
...
func (c StateTransitionCircuit) CalculateAggregatorWitness(api frontend.API, isRealVote []frontend.Variable) (groth16.Witness[sw_bw6761.ScalarField], error) {
	...
	// number of real votes as public input
	...
	for i := range params.VotesPerBatch {
		...
		hashes = append(hashes, api.Select(isRealVote[i], inputsHash, dummyProofInputsHash))
	}
...
func (circuit StateTransitionCircuit) VerifyMerkleTransitions(api frontend.API, isRealVote []frontend.Variable) {
	for i := range params.VotesPerBatch {
		...
		circuits.AssertTrueIf(api, isReal, circuit.VotesProofs.Ballot[i].IsInsertOrUpdate(api))
		circuits.AssertTrueIf(api, isReal, circuit.VotesProofs.VoteIDs[i].IsInsert(api))
...
func (c StateTransitionCircuit) VerifyMerkleCensusProofs(api frontend.API, isRealVote []frontend.Variable) {
	...
	for i := range params.VotesPerBatch {
		...
		// assert that the vote BallotIndex matches the expected derivation from the census leaf index
...
func (c StateTransitionCircuit) VerifyCSPCensusProofs(api frontend.API, isRealVote []frontend.Variable) {
	...
	for i := range params.VotesPerBatch {
		...
		circuits.AssertTrueIf(api, shouldBeValid, isValid)
		// assert that the vote BallotIndex matches the expected derivation from the census Index
		ballotIndex := BallotIndex(api, cspProof.CensusIndex)
		circuits.AssertIsEqualIf(api, shouldBeValid, ballotIndex, vote.BallotIndex)
	}
}
