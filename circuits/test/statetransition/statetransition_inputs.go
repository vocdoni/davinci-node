	...
	case origin.IsCSP():
		...
		for i := range params.VotesPerBatch {
			...
			if i < len(votes) {
				...
				votes[i].BallotIndex = types.CalculateBallotIndex(cspProof.CensusIndex)
...
func CensusIMTForTest(votes []*state.Vote) (*imtcensus.CensusIMT, error) {
	...
	for _, v := range votes {
		...
		censusIndex := ballotIndex - params.BallotMin
		entriesByIndex[censusIndex] = struct {
			address common.Address
			weight  *big.Int
		}{
		...
		if censusIndex > maxIndex {
			maxIndex = censusIndex
		}
	}
	...
	for i := uint64(0); i <= maxIndex; i++ {
		...
		if !ok {
			// Fill gaps so generated Merkle proof indexes remain aligned with census index.
