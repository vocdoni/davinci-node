...
func TestProcessProofKeysPinned(t *testing.T) {
	...
	invalid := NewTransitionWithOverwrittenVotes(t, types.CensusOriginMerkleTreeOffchainStaticV1)
	invalid.ProcessProofs.CensusOrigin = invalid.ProcessProofs.EncryptionKey

	assert := test.NewAssert(t)
	assert.CheckCircuit(
		&CircuitProcessProofKeys{*CircuitPlaceholderWithProof(&assignment.AggregatorProof, &assignment.AggregatorVK)},
...
func TestSupportedCensusOrigin(t *testing.T) {
	...
	assert := test.NewAssert(t)
	assert.CheckCircuit(
		&CircuitSupportedCensusOrigin{*CircuitPlaceholderWithProof(&assignment.AggregatorProof, &assignment.AggregatorVK)},
