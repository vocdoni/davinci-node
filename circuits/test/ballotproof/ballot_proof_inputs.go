...
func CompileAndGenerateProofForTest(inputs []byte) (string, string, error) {
	...
	log.Debugw("circom inputs parsed", "duration", time.Since(start))
	...
	log.Debugw("circom witness calculator initialized", "duration", time.Since(stepStart))
	// calculate witness
	stepStart = time.Now()
	...
	log.Debugw("circom witness calculated", "duration", time.Since(stepStart))
	// generate proof
	stepStart = time.Now()
	proof, pubInputs, err := prover.Groth16ProverRaw(TestCircomProvingKey, w)
	if err != nil {
		return "", "", err
	}
	log.Debugw("groth16 proof generated",
...
func BallotProofForTestDeterministic(address []byte, processID types.ProcessID, encryptionKey ecc.Point, seed int64) (*VoterProofResult, error) {
	...
	log.Infow("ballot proof generation finished", "duration", time.Since(now))
