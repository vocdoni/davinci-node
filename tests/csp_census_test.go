...
func TestCSPCensus(t *testing.T) {
	...
	c.Run("create process", func(c *qt.C) {
		...
		censusRoot, censusURI, signers, err = helpers.TestCensusWithRandomVoters(censusCtx, types.CensusOriginCSPEdDSABN254V1, numVoters)
		...
		pid, encryptionKey, stateRoot, err = helpers.TestNewProcess(services.Contracts, services.HTTPClient, types.CensusOriginCSPEdDSABN254V1, censusURI, censusRoot, defaultBallotMode)
		c.Assert(err, qt.IsNil, qt.Commentf("Failed to create process in sequencer"))

		// now create process in contracts
		onchainPID, err := helpers.TestProcessOnChain(services.Contracts, types.CensusOriginCSPEdDSABN254V1, censusURI, censusRoot, defaultBallotMode, encryptionKey, stateRoot, numVoters)
	...
	c.Run("create votes", func(c *qt.C) {
		for i, signer := range signers {
			...
			vote.CensusProof, err = helpers.TestCensusProof(types.CensusOriginCSPEdDSABN254V1, pid.Marshal(), signers[i].Address().Bytes())
