...
func TestOverwriteVotes(t *testing.T) {
	...
	c.Run("overwrite valid votes", func(c *qt.C) {
		...
		for i, signer := range signers {
			...
			vote, err := helpers.TestNewVoteWithRandomFields(pid, defaultBallotMode, encryptionKey, signer, nil)
			c.Assert(err, qt.IsNil, qt.Commentf("Failed to create vote"))
	...
	c.Run("wait for settled overwrite votes", func(c *qt.C) {
		...
		if err := helpers.TestWaitForWithChannel(timeoutCh, 10*time.Second, func() bool {
			// Check that votes are settled (state transitions confirmed on blockchain)
			allSettled, failed, err := helpers.TestEnsureVotesStatus(services.HTTPClient, pid, voteIDs, storage.VoteIDStatusName(storage.VoteIDStatusSettled))
			...
			votersCount, err := helpers.TestProcessVotersCountOnChain(services.Contracts, pid)
			c.Assert(err, qt.IsNil, qt.Commentf("Failed to get published votes from contract"))
			overwrittenVotes, err := helpers.TestProcessOverwrittenVotesCountOnChain(services.Contracts, pid)
