...
func TestMaxVoters(t *testing.T) {
	...
	c.Run("create process", func(c *qt.C) {
		...
		censusRoot, censusURI, signers, err = helpers.TestCensusWithRandomVoters(censusCtx, types.CensusOriginMerkleTreeOffchainStaticV1, totalVoters)
		...
		onchainPID, err := helpers.TestProcessOnChain(services.Contracts, types.CensusOriginMerkleTreeOffchainStaticV1, censusURI, censusRoot, defaultBallotMode, encryptionKey, stateRoot, initialVoters)
	...
	c.Run("handle maxVoters reached", func(c *qt.C) {
		...
		vote, err := helpers.TestNewVoteWithRandomFields(pid, defaultBallotMode, encryptionKey, extraSigner, nil)
		c.Assert(err, qt.IsNil, qt.Commentf("Failed to create vote"))
		// generate census proof for the participant
		vote.CensusProof, err = helpers.TestCensusProof(types.CensusOriginMerkleTreeOffchainStaticV1, pid.Marshal(), extraSigner.Address().Bytes())
		c.Assert(err, qt.IsNil, qt.Commentf("Failed to generate census proof"))
		...
		c.Run("update maxVoters", func(c *qt.C) {
			...
			err = helpers.TestUpdateMaxVotersOnChain(services.Contracts, pid, totalVoters)
