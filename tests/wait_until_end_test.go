...
func TestWaitUntilEnd(t *testing.T) {
	...
	c.Run("create process", func(c *qt.C) {
		...
		pid2, err := createProcessInContracts(services.Contracts, testCensusOrigin(), censusRoot, ballotMode, encryptionKey, stateRoot, time.Minute)
	...
	c.Run("wait for publish votes", func(c *qt.C) {
		t.Log("Waiting for process to end...")
		err = waitUntilProcessEnds(services.Contracts, pid, 5*time.Minute)
		c.Assert(err, qt.IsNil, qt.Commentf("Process did not reach Ended status in time"))
