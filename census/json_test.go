...
func TestJSONImporterValidURI(t *testing.T) {
	...
	c.Assert(JSONImporter.ValidURI("http://example.com/dump"), qt.IsTrue)
	c.Assert(JSONImporter.ValidURI("https://example.com/dump"), qt.IsTrue)
	c.Assert(JSONImporter.ValidURI("ftp://example.com/dump"), qt.IsFalse)
	c.Assert(JSONImporter.ValidURI("file:///tmp/dump"), qt.IsFalse)
	c.Assert(JSONImporter.ValidURI("HTTPS://example.com/dump"), qt.IsFalse)
}
...
func TestRequestRawDump(t *testing.T) {
	...
	c.Run("SetsAcceptHeader", func(c *qt.C) {
		...
		res, err := requestRawDump("https://example.invalid/census")
		c.Assert(err, qt.IsNil)
	...
	c.Run("Non200IncludesBody", func(c *qt.C) {
		...
		res, err := requestRawDump("https://example.invalid/missing")
	...
	c.Run("DoError", func(c *qt.C) {
		...
		res, err := requestRawDump("https://example.invalid/fail")
		c.Assert(res, qt.IsNil)
		c.Assert(err, qt.Not(qt.IsNil))
...
func TestDownloadAndImportCensus(t *testing.T) {
	c := qt.New(t)
	...
	c.Run("Success", func(c *qt.C) {
		...
		err := JSONImporter.DownloadAndImportCensus(censusDB, "https://example.invalid/dump.json", expectedRoot)
		c.Assert(err, qt.IsNil)
	})
	...
	c.Run("DownloadErrorWrapped", func(c *qt.C) {
		...
		err := JSONImporter.DownloadAndImportCensus(censusDB, "https://example.invalid/dump", expectedRoot)
		c.Assert(err, qt.Not(qt.IsNil))
		c.Assert(err.Error(), qt.Contains, "failed to download JSON dump")
	})
	...
	c.Run("ImportErrorWrapped", func(c *qt.C) {
		...
		err := JSONImporter.DownloadAndImportCensus(censusDB, "https://example.invalid/dump.jsonl", expectedRoot)
		c.Assert(err, qt.Not(qt.IsNil))
