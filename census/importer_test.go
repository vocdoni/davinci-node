...
func (p *testImporterPlugin) DownloadAndImportCensus(censusDB *censusdb.CensusDB, targetURI string, expectedRoot types.HexBytes) error {
	p.calls++
...
func TestCensusImporter(t *testing.T) {
	...
	c.Run("Validation", func(c *qt.C) {
		c.Run("NilCensus", func(c *qt.C) {
			...
			err := importer.ImportCensus(context.Background(), nil)
		...
		c.Run("InvalidOrigin", func(c *qt.C) {
			importer := NewCensusImporter(nil)
			err := importer.ImportCensus(context.Background(), &types.Census{
	...
	c.Run("MerkleTree", func(c *qt.C) {
		c.Run("SelectsFirstMatchingPlugin", func(c *qt.C) {
			...
			err := importer.ImportCensus(context.Background(), census)
